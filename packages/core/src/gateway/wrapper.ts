...
import type { TaskCategory, ChatMessage } from "../providers/types.js";
...
export class Gateway {
  ...
  private async initLoaders(): Promise<void> {
    const mitreDataPath = path.join(this.dataDir, "mitre", "attack-stix-data");
    const scfDataPath = path.join(this.dataDir, "scf");
  ...
  private parseWebhookMessage(channelType: string, body: unknown): { content: string; sender?: string; timestamp?: number } | null {
    ...
    switch (channelType) {
      case 'telegram':
        // Telegram: { message: { text, from: { first_name, username } } }
        const tgMsg = data.message as Record<string, unknown> | undefined;
        if (tgMsg) {
          content = String(tgMsg.text || '');
          const from = tgMsg.from as Record<string, unknown> | undefined;
          sender = from ? String(from.first_name || from.username || 'Unknown') : undefined;
        }
        break;
        
      case 'discord':
        // Discord: { content, author: { username } }
        content = String(data.content || '');
        const discordAuthor = data.author as Record<string, unknown> | undefined;
        sender = discordAuthor ? String(discordAuthor.username || 'Unknown') : undefined;
        break;
        
      case 'slack':
        // Slack: { event: { text, user } }
        const slackEvent = data.event as Record<string, unknown> | undefined;
        if (slackEvent) {
          content = String(slackEvent.text || '');
          sender = String(slackEvent.user || 'Unknown');
        }
        break;
        
      case 'feishu':
        // Feishu: { message: { body: { text } }, sender: { sender_id: { name } } }
        const feishuMsg = data.message as Record<string, unknown> | undefined;
        if (feishuMsg) {
          const feishuBody = feishuMsg.body as Record<string, unknown> | undefined;
          content = String(feishuBody?.text || '');
          const feishuSender = data.sender as Record<string, unknown> | undefined;
          if (feishuSender) {
            const senderId = feishuSender.sender_id as Record<string, unknown> | undefined;
            sender = senderId ? String(senderId.name || 'Unknown') : undefined;
          }
        }
        break;
        
      case 'msteams':
        // Teams: { text, from: { name } }
        content = String(data.text || '');
        const teamsFrom = data.from as Record<string, unknown> | undefined;
        sender = teamsFrom ? String(teamsFrom.name || 'Unknown') : undefined;
        break;
        
      case 'line':
        // LINE: { events: [{ message: { text }, source: { userId } }] }
        const lineEvents = data.events as Array<Record<string, unknown>> | undefined;
        if (lineEvents && lineEvents.length > 0) {
          const lineEvent = lineEvents[0];
          const lineMsg = lineEvent.message as Record<string, unknown> | undefined;
          content = String(lineMsg?.text || '');
          const lineSource = lineEvent.source as Record<string, unknown> | undefined;
          sender = lineSource ? String(lineSource.userId || 'Unknown') : undefined;
        }
        break;
  ...
  private detectTaskCategory(query: string, skill?: string): TaskCategory {
    ...
    if (skill) {
      ...
      if (skillRouting[skill]) return skillRouting[skill];
    }
  ...
  async start(): Promise<void> {
    ...
    this.httpServer = http.createServer(async (req, res) => {
      ...
      // Role-LLM Configuration API
      if (url.pathname === "/api/roles/llm-config" && req.method === "GET") {
        const configs = this.configManager.get("roleLLMConfigs") || [];
        res.writeHead(200, headers);
        res.end(JSON.stringify({ configs }));
        return;
      }
      
      if (url.pathname === "/api/roles/llm-config" && req.method === "POST") {
        try {
          const body = await this.readRequestBody(req);
          const { configs } = (body || {}) as { configs?: Array<{ roleId: string; providerId: string; modelName: string }> };
          if (configs) {
            await this.configManager.set("roleLLMConfigs", configs);
          }
          res.writeHead(200, headers);
          res.end(JSON.stringify({ success: true }));
        } catch (error) {
      ...
      if (url.pathname === "/api/channels/connect" && req.method === "POST") {
        try {
          const body = await this.readRequestBody(req);
          const { type, config } = (body || {}) as { type?: string; config?: Record<string, unknown> };
          if (!type) {
            res.writeHead(400, headers);
            res.end(JSON.stringify({ success: false, error: "Channel type required" }));
            return;
          }
      ...
      if (url.pathname === "/api/channels/disconnect" && req.method === "POST") {
        try {
          ...
          const { channelService } = await import('../services/channel-service.js');
          const result = await channelService.disconnectChannel(type);
          res.writeHead(200, headers);
          res.end(JSON.stringify(result));
        } catch (error) {
          res.writeHead(500, headers);
      ...
      if (url.pathname === "/api/channels/send" && req.method === "POST") {
        try {
          const body = await this.readRequestBody(req);
          const { type, content, targetId } = (body || {}) as { type?: string; content?: string; targetId?: string };
          if (!type || !content) {
            res.writeHead(400, headers);
            res.end(JSON.stringify({ success: false, error: "Type and content required" }));
            return;
          }
          const { channelService } = await import('../services/channel-service.js');
          const result = await channelService.sendMessage(type, content, targetId);
          res.writeHead(result.success ? 200 : 400, headers);
          res.end(JSON.stringify(result));
        } catch (error) {
          res.writeHead(500, headers);
      ...
      if (url.pathname === "/api/channels/status" && req.method === "GET") {
        try {
          const { channelService } = await import('../services/channel-service.js');
          ...
          res.writeHead(500, headers);
          res.end(JSON.stringify({ channels: [], error: error instanceof Error ? error.message : "Unknown error" }));
        }
        return;
      }

      // Webhook endpoints for communication platforms
      if (url.pathname.startsWith("/webhook/") && (req.method === "POST" || req.method === "GET")) {
        const channelType = url.pathname.split('/')[2];
        try {
          const body = req.method === "POST" ? await this.readRequestBody(req) : {};
          
          // Parse platform-specific message formats
          const message = this.parseWebhookMessage(channelType, body);
          ...
          if (message) {
            ...
            console.log(`[Webhook] Broadcasting message from ${channelType}:`, message);
          }
          
          res.writeHead(200, headers);
          res.end(JSON.stringify({ received: true, channel: channelType }));
        } catch (error) {
          res.writeHead(500, headers);
          res.end(JSON.stringify({ error: error instanceof Error ? error.message : "Unknown error" }));
        }
        ...
        try {
          ...
          if (provider === "ollama") {
            ...
            try {
              ...
              if (response.ok) {
                res.writeHead(200, headers);
                res.end(JSON.stringify({ success: true, message: "API Key验证成功" }));
              } else if (response.status === 401) {
                res.writeHead(200, headers);
                res.end(JSON.stringify({ success: false, message: "API Key无效" }));
              } else {
                res.writeHead(200, headers);
                res.end(JSON.stringify({ success: false, message: `验证失败: HTTP ${response.status}` }));
              }
            } catch (e) {
              res.writeHead(200, headers);
              res.end(JSON.stringify({ success: false, message: `连接失败: ${e instanceof Error ? e.message : "网络错误"}` }));
            }
            ...
            // Anthropic doesn't have a simple validation endpoint, just check key format
            if (testKey.startsWith("sk-ant-")) {
              res.writeHead(200, headers);
              res.end(JSON.stringify({ success: true, message: "API Key格式正确" }));
            } else {
              res.writeHead(200, headers);
            ...
            if (testKey.length > 10) {
              res.writeHead(200, headers);
              res.end(JSON.stringify({ success: true, message: "API Key已配置" }));
            } else {
              res.writeHead(200, headers);
          ...
          // MiniMax uses group_id + api_key format
          // API Key format: eyJhbGci... (JWT token) or simple key
          ...
          // Generic provider - just check if configured
          const hasConfig = !!(baseUrl || apiKey);
          res.writeHead(200, headers);
          res.end(JSON.stringify({ 
            success: hasConfig, 
            message: hasConfig ? "配置已就绪" : "请先完成配置" 
          }));
        }
        } catch (error) {
          res.writeHead(500, headers);
